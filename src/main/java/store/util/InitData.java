...
public class InitData {
    ...
    public InitData(PromotionRepository promotionRepository, ProductRepository productRepository) {
    ...
    private void saveProducts() throws IOException {
        ...
        for (List<String> list : mainList) {
            Product product = Product.of(
                    ...
                    list.get(1),
                    list.get(2),
                    promotionRepository.findPromotionByName(list.get(3))
            );

            productRepository.addProduct(product);

            // TODO : 나도 이렇게 하고 싶지는 않았는데...
            if("오렌지주스".equals(list.get(0)) || "탄산수".equals(list.get(0))){
